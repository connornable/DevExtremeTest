  ...
  constructor() { 
    ...
    async initClients() {
      console.log("this was called");
        ...
        .then(data => {console.log(data.data.clients[0].name); this.clients = data.data.clients})
        .then(() => console.log(this.clients))
        .then(() => this.clients.forEach((element) => {
          if (!this.filteredClients.some((e) => e.name === element.name)) {
         this.filteredClients.push(new Client(this.clients.indexOf(element), element.name));
          }
        }))
